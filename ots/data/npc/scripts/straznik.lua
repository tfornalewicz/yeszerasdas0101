...
local config = {
    ...
    attackPK = {value = false, skulls = {SKULL_WHITE, SKULL_RED}},
    attackMonster = {value = true, ignore = {"Cat", "Dog"}, ignoreSummon = true},
    damageValue = {min = 50, max = 100}
}
...
function onThink()
	...
	if number == 1 then
		...
		doTargetCombatHealth(npcId, targetId, COMBAT_DEATHDAMAGE, -config.damageValue.min, -config.damageValue.max, CONST_ME_SMALLCLOUDS)
		npcPosition:sendDistanceEffect(targetPosition, CONST_ANI_SUDDENDEATH)
