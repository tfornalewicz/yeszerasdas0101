...
local voices = {
  ...
  if storage == 1 then
        ...
        --Cup of a molten gold
        elseif msgcontains(msg, "melt") then talkState[talkUser] = 70
            npcHandler:say("Moge przetopic dla ciebie to zloto, czy na pewno mam to wykonac?", cid)
        --Help
        elseif msgcontains(msg, "help") then
            npcHandler:say("I can forge Amulet, Huge Chunk of Crude Iron, Piece of Draconian Steel, Piece of Royal Steel, Piece of Hell Steal and Infernal Bolts!", cid)
        --No
        ...
        elseif msgcontains(msg, "yes") then
            ...
            --Cup of a molten gold
            elseif talkState[talkUser] == 70 then
                if getPlayerItemCount(cid,9971) >= 1 then
                    doPlayerRemoveItem(cid,9971,1)
                    doPlayerAddItem(cid,13941,1)
                    npcHandler:say("Cling clang!", cid)
                else
                    npcHandler:say("Wroc jesli zdobedziesz niezbedne przedmioty.", cid)
                end
            --Amulet  
            elseif talkState[talkUser] == 10 then
                if getPlayerItemCount(cid,2152) >= 50 and getPlayerItemCount(cid,8262) >= 1 and getPlayerItemCount(cid,8263) >= 1 and getPlayerItemCount(cid,8264) >= 1 and  getPlayerItemCount(cid,8265) >= 1 then
