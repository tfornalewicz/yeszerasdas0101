...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "mission") or msgcontains(msg, "report") then
		...
		elseif player:getStorageValue(Storage.TheInquisition.Questline) == 1 then
			npcHandler:say({
				...
				"Follow the instructions in the guide to talk to the guards that protect the cities and test their loyalty. Then report to me about your {mission}."
			}, cid)
		...
		elseif player:getStorageValue(Storage.TheInquisition.Questline) == 3 then
			npcHandler:say({
				"Listen, we have information about a heretic coven that hides in swamp located south of Mirko Town. The witches reach this cursed place by hidden underground entrance and think they are safe there. ...",
		...
		elseif player:getStorageValue(Storage.TheInquisition.Questline) == 5 then
			if player:removeItem(8702, 1) then
				npcHandler:say({
					...
					"You'll find him in his cave deep in dwarven mines. He'll tell you about your next mission."
				}, cid)
		...
		elseif player:getStorageValue(Storage.TheInquisition.Questline) == 11 then
			npcHandler:say({
				"We've got a report about an abandoned pirate ruins in jungle. I want you to examine these ruins. ...",
		...
		elseif player:getStorageValue(Storage.TheInquisition.Questline) == 12 or player:getStorageValue(Storage.TheInquisition.Questline) == 13 then
			npcHandler:say("Your current mission is to exorcise an evil being from a pirate ruins. Are you done with that mission? ", cid)
		...
		elseif player:getStorageValue(Storage.TheInquisition.Questline) == 15 then
			if player:removeItem(6500, 20) then
				npcHandler:say({
					...
					"I also have a reward for your great efforts. Talk to me about your {elementalist outfit} anytime from now on. Afterwards, let's talk about the next mission that's awaiting you."
				}, cid)
		...
		elseif player:getStorageValue(Storage.TheInquisition.Questline) == 17 then
			npcHandler:say({
				"We've got information about something very dangerous going on the desert. The demons are preparing something there ...",
				...
				"It'll be your task to take revenge and to kill that demon. You'll find him in the depths of desert demon lair. Good luck."
			}, cid)
		...
		elseif player:getStorageValue(Storage.TheInquisition.Questline) == 19 then
			...
			player:addOutfitAddon(433, 1)
			player:addOutfitAddon(432, 1)
		...
		elseif npcHandler.topic[cid] == 6 then
			if player:getStorageValue(Storage.TheInquisition.Questline) == 22 then
				npcHandler:say({
					...
					"The blessing of the inquisition will bestow upon you all available blessings for the price of 60000 gold. Also, don't forget to ask me about your {outfit} to receive the final addon as elementalist."
				}, cid)
	...
	elseif msgcontains(msg, "outfit") then
		if player:getStorageValue(Storage.TheInquisition.Questline) == 16 then
			npcHandler:say("Here is your elementalist outfit. You deserve it. Unlock more addons by completing more missions.", cid)
			...
			player:addOutfit(433, 0)
			player:addOutfit(432, 0)
			player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
			npcHandler.topic[cid] = 0
		elseif player:getStorageValue(Storage.TheInquisition.Questline) == 23 then
			npcHandler:say("Here is the final addon for your elementalist outfit. Congratulations!", cid)
			...
			player:addOutfitAddon(433, 2)
			player:addOutfitAddon(432, 2)
			player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
	...
	elseif IsinArray({'kult', 'lizardy', 'brama', 'przyzwanie'}, msg) then
		npcHandler:say({
			'Pytasz sie mnie jak mozna by przerwac czynnosci wzywajace diabla? Czyli Jaszczuroludzie znowu cos kombinuja. Hmmm, nie jestem w stanie rozdzielic sie i ratowac swiat na dwa fronty! ...',
			'Najwazniejsze to dowiedziec sie na jakim etapie sa. Trzeba przeszkodzic im w przygotowaniach jak i w modlach. Zlokalizuj ich oltarz, lub miejsce kultu. Mysle, ze zaklocisz ich medytacje, jesli zrobisz tam duzy balagan. Sproboj rozpalic ognisko na stodku miejsca ich kultu. To powinno starczyc.'
		}, cid)
		npcHandler.topic[cid] = 0
	end
	return true
